...
* How does the Hodges-Le Cam estimator behave asymptotically?
...
* Fisher's 2 conjectures.

#### **Construction of the Hodges-Le Cam estimator.**

The construction of the Hodges-Le Cam estimator is as follows.

Consider $$n$$ independent and identically distributed random variables, $$X_1, \dots, X_n \sim N(\theta, 1/n)$$. The maximum likelihood estimator of the mean parameter $$\theta$$ is $$\bar{X}_n$$. Define the Hodges-Le Cam estimator as

$$
\tilde{\theta}_n =
\begin{cases}
\space 0 \quad &\text{if} \quad \vert \bar{X}_n \vert \leq n^{-1/4} \\
\space \bar{X}_n \quad &\text{if} \quad \vert \bar{X}_n \vert > n^{-1/4}
\end{cases}
$$

The first thing to note here is that $$\tilde{\theta}_n$$ is a *mixed random variable* in that it has *both* discrete and continuous components. The construction places point mass at $$0$$, zero mass in the interval $$[-n^{-1/4}, n^{-1/4}]$$, and has positive density in the interval $$(-n^{-1/4}, n^{-1/4})$$.

Second, because $$X_1, \dots, X_n \sim N(\theta, 1)$$, we have that $$\bar{X}_n \sim N(\theta, 1/n)$$.

In order to reveal the significance of the construction $$\tilde{\theta}_n$$, we begin by inquiring how it behaves asymptotically, that is, as the sample size $$n \rightarrow \infty$$.

#### **How does the Hodges-Le Cam estimator behave asymptotically?**

One way to answer how $$\tilde{\theta}_n$$ behaves asymptotically is by computing its limiting distribution. The two cases of interest are when $$\theta = 0$$, and when $$\theta \neq 0$$, and we now compute the limiting distribution for both cases.

*Case 1: Limiting distribution of* $$\tilde{\theta}_n$$ *when* $$\theta = 0$$.

The key observation about $$\tilde{\theta}_n$$ when $$\theta = 0$$ is that $$P(\tilde{\theta}_n = 0) \rightarrow 1$$ and $$P(\tilde{\theta}_n = \bar{X}_n) \rightarrow 0$$.

Bearing in mind that $$P(\tilde{\theta}_n = \bar{X}_n) = P \left( \vert \bar{X}_n \vert > n^{-1/4} \right)$$, we have that

$$\begin{aligned}
    P \left( \vert \bar{X}_n \vert > n^{-1/4} \right) &= P \left( \{ \bar{X}_n > n^{-1/4} \} \cup \{ \bar{X}_n < -n^{-1/4} \}\right) \\
    &= P \left( \{ \sqrt{n} \bar{X}_n > n^{-1/4} \sqrt{n} \} \cup \{ \sqrt{n} \bar{X}_n < -n^{-1/4} \sqrt{n} \}\right) \\
    &= P( \{  Z > n^{1/4}\} \cup \{ Z < -n^{1/4} \}) \\
    &\leq 2 \Phi(-n^{1/4})
\end{aligned}$$

Where the final line uses the union bound, the symmetry of the standard Normal pdf $$\phi(z)$$ and where $$\Phi(z) = \int \phi(z) dz$$.

The properties of the standard normal CDF mean that as $$n \rightarrow \infty$$, we have that $$2 \Phi(-n^{1/4}) \rightarrow 0$$, and therefore, $$P( \vert \bar{X}_n \vert > n^{-1/4} \rightarrow 0$$. Yielding the result that as $$n \rightarrow \infty$$,

$$P(\tilde{\theta}_n = 0) = P \left( \vert \bar{X}_n \vert \leq n^{-1/4} \right) \longrightarrow 1.$$

Notice that the estimator $$\tilde{\theta}_n$$ can be written as $$\tilde{\theta}_n = \bar{X}_n A_n $$ where $$A_n = \mathbb{I}(\vert \bar{X}_n \vert > n^{-1/4})$$ is an indicator function. Now because $$\bar{X}_n \sim N(0, 1/n)$$, we have that $$\bar{X}_n \overset{d}\rightarrow 0$$. Furthermore, because $$P \left( \vert \bar{X}_n \vert > n^{-1/4} \right) \rightarrow 1$$, $$A_n \overset{d}{\rightarrow} 0$$. Via Slutsky's theorem, we have that the limiting distribution of $$\tilde{\theta}_n$$ when $$\theta = 0$$ is given by

$$\tilde{\theta}_n \overset{d}{\longrightarrow} 0.$$

*Case 2: Limiting distribution of* $$\tilde{\theta}_n$$ *when* $$\theta \neq 0$$.

We have that when $$\theta \neq 0$$, $$P(\tilde{\theta}_n = 0) \rightarrow 0$$ and $$P(\tilde{\theta}_n = \bar{X}_n) \rightarrow 1$$.

Bearing in mind that $$P(\tilde{\theta}_n = 0) = P( \vert \bar{X}_n \vert \leq n^{-1/4})$$, we have that

$$\begin{aligned}
     P \left( \vert \bar{X}_n \vert \leq n^{-1/4} \right) &= P \left( -n^{-1/4} \leq \bar{X}_n \leq n^{-1/4}  \right) \\
     &= P \left(\sqrt{n}(-n^{-1/4} - \theta) \leq Z \leq \sqrt{n}(n^{-1/4} - \theta) \right) \\
     &= \Phi \left(n^{1/4} - \theta \sqrt{n} \right) - \Phi \left(-n^{1/4} - \theta \sqrt{n} \right) \\
\end{aligned}$$

For both $$\theta > 0$$ and $$\theta < 0$$, notice that $$-\theta \sqrt{n}$$ dominates.

In the case where $$\theta > 0$$, then we have that $$n^{1/4} - \theta \sqrt{n} \rightarrow -\infty$$ and $$-n^{1/4} - \theta \sqrt{n} \rightarrow - \infty$$. Using the property of the standard normal CDF that $$\Phi(t) \rightarrow 0$$ as $$t \rightarrow -\infty$$, we have that $$P \left( \vert \bar{X}_n \vert \leq n^{-1/4} \right) \rightarrow 0$$.

In the case where $$\theta < 0$$, then we have that $$n^{1/4} - \theta \sqrt{n} \rightarrow \infty$$ and $$-n^{1/4} - \theta \sqrt{n} \rightarrow \infty$$. Using the property of the standard normal CDF that $$\Phi(t) \rightarrow 1$$ as $$t \rightarrow 1$$, we have that $$P \left( \vert \bar{X}_n \vert \leq n^{-1/4} \right) \rightarrow 0$$.

Hence for $$\theta \neq 0$$, we have that as $$n \rightarrow \infty$$, 

$$P(\tilde{\theta}_n = \bar{X}_n) = P \left( \vert \bar{X}_n \vert > n^{-1/4} \right) \longrightarrow 1.$$

Similarly as before, the estimator $$\tilde{\theta}_n$$ can be written as $$\tilde{\theta}_n = \bar{X}_n A_n$$ where $$A_n = \mathbb{I}(\vert \bar{X}_n \vert > n^{-1/4})$$. In this case, we have that $$$$ Now because $$P \left( \vert \bar{X}_n \vert > n^{-1/4} \right) \rightarrow 1$$, we have that $$A_n \overset{d}{\rightarrow} 1$$. 
